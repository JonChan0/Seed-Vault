...
| `.claude/agents/` Claude pipeline agents | ❌ gitignored | generated by install.sh |
...
`vault-pipeline` doesn't do new work of its own — it **orchestrates context-bounded
subagents**, parallelising steps whose context is disparate and serialising the one
that needs the whole graph. So you can process everything with a single request.

| Skill Name | Say this... | The LLM will... |
|------------|-------------|-----------------|
| `vault-pipeline` | "Process everything" / "run the pipeline" | Run pipeline.py to detect new/changed files, then spawn **parallel `source-ingestor` agents** (one per raw file) → a **single `wiki-synthesizer`** (whole-graph compile + index) → **parallel `clean-room-verifier` agents** (one per new article) → deterministic apply-fixes/lint/digest |

### Pipeline agents

The pipeline spawns subagents defined in `_vault/agents/` (installed to
`.claude/agents/`). Agents are split by **context boundary, not function**: work
with disparate per-item context runs in parallel; work needing the full wiki graph
runs alone. Each agent is a thin wrapper that calls the relevant `vault-*` skill.

| Agent | Model | Context boundary | Parallel | Calls |
|-------|-------|------------------|----------|-------|
| `source-ingestor` | Sonnet | one raw file | yes (per file) | `vault-ingest` |
| `wiki-synthesizer` | Opus | whole wiki graph | no (serial) | `vault-compile`, `vault-index` |
| `clean-room-verifier` | Opus | one article + sources (read-only) | yes (per article) | `vault-verify` |
| `visualizer` | Sonnet | one concept's data | yes (per viz) | `vault-visualize` |
| `qa-responder` | Haiku | one query + retrieved snippets | per query | `vault-qa` |

`visualizer` and `qa-responder` are standalone — spawned directly for charts and
questions, not part of the default pipeline path. See `_vault/agents/SPEC.md`.
