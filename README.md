...
# Install skills for Claude Code (symlinks) and Gemini CLI (hard links in skills/)
...
Or in **Gemini CLI**, open the folder and say the same — `GEMINI.md` and `skills/` are auto-loaded.
...
| Skills source | `_vault/vault-*/SKILL.md` | `skills/vault-*/SKILL.md` |
| Skills install | Soft symlinks → `~/.claude/skills/` | Hard links in `skills/` (workspace) |
| Setup command | `bash _vault/install.sh` then `/reload-plugins` | `bash _vault/install.sh` then `cd vault && gemini` |
